...
int BitcoinExchange::addYMD(std::string::iterator&it, std::string::iterator&it2, std::string&line, std::string&Y, char c){
...
int BitcoinExchange::checkSyntax(std::string &line){
    ...
    if ( line == "date,exchange_rate" && i == 0 )
        return i++, 0;
    if (i == 0 || addYMD(it, it2, line, y, '-') < 0 || addYMD(it, it2, line, m, '-') < 0 ||
    ...
    if (checkNum(y, 2022, 2008) < 0 || checkNum(m, 12, 0) < 0 || checkNum(d, 31, 0) < 0 || checkNum(v, INT_MAX, 0) < 0)
        return -4;
    addValue(line);
    return 0;
}
...
int BitcoinExchange::checkDM(std::string &m, std::string&d){
